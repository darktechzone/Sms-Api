...
def extract_sesskey(html, url=""):
    """Try multiple patterns to extract sesskey from HTML or URL"""
    # Pattern 1: from a URL parameter
    match = re.search(r'sesskey=([^&"\']+)', html)
    if match:
        return match.group(1)
    # Pattern 2: from a JavaScript variable assignment
    match = re.search(r'sesskey\s*[:=]\s*["\']([^"\']+)["\']', html)
    if match:
        return match.group(1)
    # Pattern 3: from a form input (name="sesskey")
    match = re.search(r'<input[^>]*name=["\']sesskey["\'][^>]*value=["\']([^"\']+)["\']', html, re.IGNORECASE)
    if match:
        return match.group(1)
    # Pattern 4: from a hidden div or data attribute
    match = re.search(r'data-sesskey=["\']([^"\']+)["\']', html)
    if match:
        return match.group(1)
    # Pattern 5: from the URL string passed (if any)
    if url:
...
def login():
    ...
    # Step 3: Access stats page and also try the dashboard page
    # First, try the stats page
    ...
    sesskey = extract_sesskey(r3.text, r3.url)
    
    # If still not found, try the main dashboard
    if not sesskey:
        print("[LOGIN] sesskey not in stats page, trying dashboard...")
        r4 = session.get(f"{BASE_URL}/agent/SMSDashboard", headers={"Referer": BASE_URL})
        sesskey = extract_sesskey(r4.text, r4.url)
    
    # If still not found, try any other agent page
    if not sesskey:
        print("[LOGIN] Trying alternative page: /agent/MySMSNumbers2")
        r5 = session.get(f"{BASE_URL}/agent/MySMSNumbers2", headers={"Referer": BASE_URL})
        sesskey = extract_sesskey(r5.text, r5.url)
    
    if not sesskey:
        # Log first 500 chars of the last response for debugging
        debug_content = (r3.text[:500] if 'r3' in locals() else '') + (r4.text[:500] if 'r4' in locals() else '')
        print(f"[LOGIN] DEBUG RESPONSE: {debug_content}")
        raise Exception("sesskey not found after login in any page")
...
# For Vercel, the app object is exported
